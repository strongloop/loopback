...
/**
 * By default Loopback doesn't support validation for all possible schema restriction.
 * Let's solve this issue
 */
Registry.prototype._applySchemaValidation = function (ModelCtor) {

  var properties = ModelCtor.definition.properties;
  var relations = ModelCtor.definition.relations;

  if(null == relations || 0 == relations.length){
    relations = ModelCtor.definition.settings.relations;
  }
  ...
  for(var key in properties){
    ...
    if(null != field.length){
      ...
      ModelCtor.validate(key, function(key,field){
        return function(err){
          if(null == this[key]) return;
          if (this[key].length > field.length) err();
        }
      }(key,field), {
        code: 'max-length',
        message: 'must be less than ' + field.length + ' charsets'
      });
    ...
    if(null != field.in){
      ...
      ModelCtor.validate(key,  function(key,field){
        return function(err){
          if(null == this[key]) return;
          if (-1 === field.in.indexOf(this[key])) err();
        }
      }(key,field), {
        code: "enum",
        message: 'must be one of the values [' + field.in.join(',') +']'
    ...
    if(field.uniq){
      ModelCtor.validatesUniquenessOf(key, {
        message: 'already in use'
      });

    }

    if(true === field.unsigned){
      ModelCtor.validate(key,  function(key,field){
        return function(err){
          if(null == this[key]) return;
          if (this[key]*1 < 0) err();
        ...
        code: 'unsigned-number',
        message: 'must be greater than zero'
      });
    }

    if('Number' == field.type.name){
      ModelCtor.validate(key, function(key,field){
        return function(err){
          if(null == this[key]) return;
          if (isNaN(this[key])) err();
        }
      }(key, field), {
        code: 'number',
    ...
    if('Date' === field.type.name){
      ...
      ModelCtor.validate(key, function(key,field){
        ...
        return function(err){
          ...
          if(null == this[key]) return;
          var unixTime = new Date(this[key]);

          if (isNaN(unixTime) || unixTime.getTime() <= 0) err();
  ...
  for(var key in relations){
    ...
    var fieldKey = key + 'Id';
    var field = properties[fieldKey];

    ModelCtor.validateAsync(fieldKey, function(key, field){

      var fieldKey = key + 'Id';

      return function(err, done){

        if(null != this[fieldKey] && !isNaN(this[fieldKey])){

          this[key].getAsync(function(errRel, relatedObject){
            if(errRel) throw errRel;

            if(null == relatedObject){
              err('external-model-not-found');
            }
...
Registry.prototype.createModel = function(name, properties, options) {
  ...
  this._applySchemaValidation(model);

  if(BaseModel == this.getModel('PersistedModel')){
      model._onContextRestrictionListener();
  }
