...
/**
 * Get the children Models extend form this parent model
 *
 * @callback {Function} cb
 * @param {Error} err
 * @param {Array} model's names
 * @end
 */

Model.resolveChild = function (cb) {
  var parentModel = this;
  parentModel.getApp(function (err, app) {
    if (err)return cb(err);
    cb(null, Object.keys(app.models).filter(function (modelName) {
      return app.models[modelName].settings.base === parentModel.definition.name;
    }));
  });
};
