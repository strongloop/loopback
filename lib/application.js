...
app.boot = function(options) {
  ...
  processAppConfig(app, appConfig);
...
function processAppConfig(app, appConfig) {
  appConfig.host =
  ...
  appConfig.loggerFormat =
    appConfig.loggerFormat ||
      (app.get('env') === 'dev' ? 'dev' : 'default');
...
app.installMiddleware = function() {
  ...
  this.use(loopback.logger(this.get('loggerFormat')));
