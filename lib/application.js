...
if (runtime.isServer) {
  // TODO(bajtos) Move this block to a new file that is excluded from browserify
  var PhasedRouter = require('./phased-router');

  // override the default router used by express
  // TODO(bajtos) contribute a customization hook to express
  // so that we don't have to keep a copy of the `lazyrouter` function
  app.lazyrouter = function() {
    ...
    // TODO(bajtos) figure out how to forward all options
    self._phasedRouter = new PhasedRouter({
      caseSensitive: self._router.caseSensitive,
      strict: self._router.strict
    });

    self._phasedRouter.use(function runRootHandlers(req, res, next) {
      self._router.__expressHandle(req, res, next);
    });

    self._router.handle = function(req, res, next) {
      self._phasedRouter.handle(req, res, next);
    };
  };
  ...
  app.phase = function(name) {
    this.lazyrouter();
    return this._phasedRouter.phase(name);
  };
}
