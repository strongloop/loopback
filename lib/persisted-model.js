...
var debugContext = require('debug')('loopback:acl-context');
...
module.exports = function(registry) {
  ...
  PersistedModel.setup = function setupPersistedModel() {
    ...
    // extract asserts from a model definition
    this._asserts = [];
    if(this.definition.settings._asserts){
      this.definition.settings._asserts.forEach(function(it){
        PersistedModel.attachContext(it.principalType + ':' + it.principalId, it.event, it.assert);
      })
    }
  };

  /**
   * Attaching an assertion for checking context
   * @param principal
   * @param event
   * @param assert
   */
  PersistedModel.attachContext = function(principal, event, assert){
    if('string' != typeof principal || 0 == principal.length){
      throw 'Principal format is incorrect. Should be a string.'
    }

    if(-1 === ['ACCESS', 'WRITE', '*'].indexOf(event)){
      throw 'Supported events for context restriction: ACCESS | WRITE'
    }

    principal = principal.split(':');
    var principalType = principal.shift();
    var principalId = principal.pop();

    var self = this;

    if(-1 === ['USER', 'ROLE'].indexOf(principalType)){
      throw 'Supported principals for context restriction: USER | ROLE'
    }


    if('$owner' == assert){

      assert = function(session, event, restriction, resolve, reject){

        /**
         This is a simple assertion
         I don't want to know anything about other restrictions here and I will not check conflicts
         */

        var userKey = (null != self.definition.properties.userId)? 'userId':'id';

        if('ACCESS' == event){
          restriction[userKey] = session.id;
          resolve();
        }

        var data = arguments[2];

        if(data[userKey] == session.id){
          resolve();
        } else {
          reject();
        }
    ...
    } else if (-1 === ['*', 'WRITE'].indexOf(event) && 'function' != typeof assert){
      throw 'Assert should be a function if event = WRITE'

    } else if(-1 === ['function', 'object'].indexOf(typeof assert)){
      throw 'Assert should be an object or a function'
    }


    this._asserts.push({
      "principalType": principalType,
      "principalId": (null == principalId)? '*':principalId,
      "event": event,
      'assert': assert
    })
  ...
  /**
   * Observe ACCESS and WRITE events and check access in the request context
   * @private
   */ 

  PersistedModel.injectContext = function(){
    ...
    Model.observe('access', function(ctx, next) {
      ...
      var session;
      if(0 == Model._asserts.length || null == (session = ctx.options.session)) {
        return next();
      }
      ...
      for(var i = Model._asserts.length-1; i >= 0; i--){
        var it = Model._asserts[i];

        if(
            ['ACCESS', '*'].indexOf(it.event) > -1 && (
                ('ROLE' == it.principalType &&  null != _.find(session.roles(), {roleId: it.principalId * 1})) ||
                ('USER' == it.principalType && session.id == it.principalId) ||
                ('$authenticated' == it.principalId)
            )
        ){

          var itRestriction = ('function' == typeof it.assert)?
              new Promise(function(resolve, reject) {
                it.assert(session, 'ACCESS', restrictions, resolve, reject, ctx)
              }):
              new Promise(function(resolve, reject) {
                merge(restrictions, it.assert)
                resolve();
              })

          q.push(itRestriction)
        }
      ...
      Promise.all(q).then(function(){
        ...
        if(Object.keys(restrictions).length > 0){
          ...
          if(0 == Object.keys(ctx.query.where).length){
            ctx.query.where = restrictions;
          }else {
            ctx.query.where = {
              and: [
                restrictions,
        ...
        debugContext(Model.modelName, JSON.stringify(restrictions))
    ...
    Model.observe('before save', function (ctx, next) {
      ...
      for(var i = 0; i < Model._asserts.length; i++){
        var it = Model._asserts[i];
        if(
            ['WRITE', '*'].indexOf(it.event) > -1 && (
                ('ROLE' == it.principalType &&  null != _.find(session.roles(), {roleId: it.principalId * 1})) ||
                ('USER' == it.principalType && session.id == it.principalId) ||
                ('$authenticated' == it.principalId)
            )
          ...
          var itRestriction =  new Promise(function(resolve, reject) {
            it.assert(session, 'WRITE', data, resolve, reject, ctx)
          })
  ...
  PersistedModel.setupRemoting = function() {
    ...
    if (options.trackChanges || options.enableRemoteReplication) {
      ...
      setRemoting(PersistedModel, 'findLastChange', {
        ...
        accessType: 'READ',
        accepts: [
          {arg: 'id', type: 'any', required: true, http: { source: 'path' }, description: 'Model id'},
          {arg: 'options', type: 'object', injectCtx: true}
        ],
