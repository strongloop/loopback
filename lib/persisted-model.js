...
module.exports = function(registry) {
  ...
  /**
   * Observe ACCESS and WRITE events and check access in request context
   * @private
   */
  PersistedModel._onContextRestrictionListener = function(){

    var loopback = require('./loopback');
    var self = this;

    this.observe('access', function(ctx, next) {

      if(0 == self._asserts.length) return next();

      var context = loopback.getCurrentContext();
      var session = null;

      if(null == context || null == (session = context.get('user'))) {
        return next();
      }
      ...
      for(var i = self._asserts.length-1; i >= 0; i--){
        var it = self._asserts[i];
        ...
        if(
          ...
          var itRestriction = ('function' == typeof it.assert)?
              new Promise(function(resolve, reject) {
                it.assert(session, 'ACCESS', restrictions, resolve, reject, ctx.query.where)
              }):
              new Promise(function(resolve, reject) {
    ...
    this.observe('before save', function (ctx, next) {
      ...
      for(var i = 0; i < self._asserts.length; i++){
        var it = self._asserts[i];
        if(
          ...
          var itRestriction =  new Promise(function(resolve, reject) {
            it.assert(session, 'WRITE', data, resolve, reject)
          })
